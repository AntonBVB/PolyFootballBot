...
class EntryEngine:
    ...
    def _build_candidate(self, outcome: TradeableOutcome, now: datetime) -> EntryCandidate | None:
        ...
        if outcome.total_volume < self.settings.min_total_volume:
            return None
        event_start_time = ensure_utc_datetime(outcome.event_start_time)
        if event_start_time is None or not (now < event_start_time <= now + timedelta(hours=self.settings.open_window_hours)):
            return None
...
class ExitEngine:
    ...
    def _handle_position(self, position: PositionRecord, now: datetime) -> None:
        ...
        if book.best_bid is None:
            ...
            return
        exit_kind = None
        opened_at = ensure_utc_datetime(position.opened_at)
        if book.best_bid >= position.entry_price + self.settings.take_profit_delta:
            exit_kind = OrderKind.TP
        elif opened_at and now >= opened_at:
            # Treat naive DB timestamps as UTC so exit monitoring never mixes naive and aware datetimes.
