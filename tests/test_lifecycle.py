...
from polyfootballbot.models import MarketFormat, OrderSide, OutcomeRole, PositionRecord, PositionStatus
...
class ExitOnlyClient(StubClient):
    def get_order_book(self, token_id: str):
        return {"bids": [{"price": 0.74, "size": 10}], "asks": [{"price": 0.75, "size": 10}]}
...
def test_exit_monitor_handles_naive_opened_at_as_utc(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("SQLITE_PATH", str(tmp_path / "bot.db"))
    monkeypatch.setenv("APP_MODE", "live")
    monkeypatch.setenv("PRIVATE_KEY", "0xabc")
    monkeypatch.setenv("FUNDER", "0xdef")
    ...
    position = PositionRecord(
        ...
        token_id="no-1",
        outcome_name="Team A",
        outcome_role=OutcomeRole.HOME,
        market_format=MarketFormat.BINARY,
        shares=5.0,
        entry_price=0.90,
        status=PositionStatus.OPEN,
        opened_at=datetime(2026, 3, 21, 12, 0, 0),
        dry_run=False,
    )

    ExitEngine(settings=settings, db=db, clob=clob)._handle_position(position, now=datetime(2026, 3, 21, 12, 1, 0, tzinfo=timezone.utc))

    assert len(client.orders) == 1
    assert client.orders[0]["side"] == OrderSide.SELL.value
    assert client.orders[0]["token_id"] == "no-1"
